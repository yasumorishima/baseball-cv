...
def plot_story(df, r2_steps, out_path):
    ...
    ax1.set_xlabel(f"Bat speed proxy — peak wrist (m/s)", fontsize=11)
...
def plot_breakdown(df, out_path):
    ...
    ax1.bar(x - w / 2, q1_z, w, label=f"Q1 (Worst body, {q1_ev:.1f} mph)",
            color="#e74c3c", alpha=0.8, edgecolor="white")
    ax1.bar(x + w / 2, q5_z, w, label=f"Q5 (Best body, {q5_ev:.1f} mph)",
    ...
    ax1.set_title(
        f"Body Mechanics: Same bat speed ({q1_bat:.1f} vs {q5_bat:.1f} m/s)\n"
        f"--> {q5_ev - q1_ev:.1f} mph exit velocity difference!",
        fontsize=11,
    )
    ...
    fig.suptitle(
        f"Efficient Hitting: Body Mechanics Explain {r2_delta * 100:.1f}% Beyond Bat Speed",
