...
def main():
    ...
    # --- Left: Braking Deviation -> Trunk Timing ---
    cols_hs = ["llb_ankle_braking_decel", "head_stability_score"]
    d_hs = df[cols_hs].dropna()

    # Find optimal braking from quadratic peak
    q_coeffs = np.polyfit(d_hs[cols_hs[0]], d_hs[cols_hs[1]], 2)
    optimal_braking = -q_coeffs[1] / (2 * q_coeffs[0])

    # Deviation from optimal vs trunk timing
    cols1 = ["llb_ankle_braking_decel", "llb_time_fs_to_peak_trunk_vel"]
    d1 = df[cols1].dropna()
    ...
    ax1.set_ylabel("Time to Peak Trunk Velocity (s)", fontsize=14)
    ax1.set_title(f"Braking Deviation → Trunk Timing\n"
