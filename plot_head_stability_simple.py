...
Left:  Knee Forward Decel -> Head Stability (r=+0.30)
Right: Head Stability -> Time to Peak Trunk Velocity (r=-0.54)
...
def main():
    ...
    # --- Left: Knee Forward Decel -> Head Stability ---
    cols1 = ["llb_knee_forward_decel", "head_stability_score"]
    d1 = df[cols1].dropna()
    x1, y1 = d1[cols1[0]].values, d1[cols1[1]].values
    r1, p1 = stats.pearsonr(x1, y1)

    ax1.scatter(x1, y1, s=70, c="#2980b9", alpha=0.7,
                edgecolors="white", linewidth=0.5)
    z1 = np.polyfit(x1, y1, 1)
    xline1 = np.linspace(x1.min(), x1.max(), 50)
    ax1.plot(xline1, np.polyval(z1, xline1), color="#e74c3c", linewidth=3)

    p1_str = "p < 0.001" if p1 < 0.001 else f"p = {p1:.3f}"
    ax1.set_xlabel("Knee Forward Deceleration (m/s\u00b2)", fontsize=14)
    ax1.set_ylabel("Head Stability Score", fontsize=14)
    ax1.set_title(f"Knee Braking \u2192 Head Stability\n"
                  f"r = {r1:+.2f}  ({p1_str}, n = {len(d1)})",
    ...
    d2 = df[cols2].dropna()
    x2, y2 = d2[cols2[0]].values, d2[cols2[1]].values
    r2, p2 = stats.pearsonr(x2, y2)

    ax2.scatter(x2, y2, s=70, c="#27ae60", alpha=0.7,
                edgecolors="white", linewidth=0.5)
    ...
    ax2.set_title(f"Head Stability \u2192 Faster Trunk Rotation\n"
                  f"r = {r2:+.2f}  ({p2_str}, n = {len(d2)})",
                  fontsize=15, fontweight="bold")
    ax2.tick_params(labelsize=12)

    fig.suptitle("Movement Quality Chain \u2014 Driveline OBP",
                 fontsize=16, fontweight="bold", y=1.02)
    fig.tight_layout()
    fig.savefig(str(OUTPUT_DIR / "head_stability_scatter.png"),
